...
/**
 * A plugin to wrap the estree in `Content`.
 *
...
export function recmaDocument(options = {}) {
  ...
  return (tree, file) => {
    ...
    for (child of tree.body) {
      ...
      // ```
        ...
        child.specifiers = child.specifiers.filter((specifier) => {
          if (specifier.exported.name === 'default') {
            ...
            // Make it just an import: `import Layout from '…'`.
            handleEsm(
              create(specifier, {
                type: 'ImportDeclaration',
                specifiers: [
      ...
      // ```
        ...
        content = true;
        replacement.push(...createContent(child.expression, Boolean(layout)));
    ...
    if (!content) {
      replacement.push(...createContent(undefined, Boolean(layout)));
    }

    exportedIdentifiers.push(['Content', 'default']);
   ...
   */
  function createContent(content, hasInternalLayout) {
    /** @type {JSXElement} */
    const element = {
      type: 'JSXElement',
      openingElement: {
      ...
      children: [
        {
          ...
          openingElement: {
            type: 'JSXOpeningElement',
            name: { type: 'JSXIdentifier', name: '_createContent' },
            attributes: [
              {
                type: 'JSXSpreadAttribute',
                argument: { type: 'Identifier', name: 'props' },
    ...
    if (!hasInternalLayout) {
      // result = {
      //   type: 'CallExpression',
      //   callee: { type: 'Identifier', name: '_createContent' },
      //   arguments: [{ type: 'Identifier', name: 'props' }],
      //   optional: false,
      // };
      result = {
        type: 'ConditionalExpression',
        test: { type: 'Identifier', name: 'Layout' },
        consequent: result,
        alternate: {
          type: 'CallExpression',
          callee: { type: 'Identifier', name: '_createContent' },
    ...
    return [
      {
        ...
        id: { type: 'Identifier', name: '_createContent' },
