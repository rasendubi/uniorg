 ...
 * `props.components` (when the function is called `_createContent`), or from
  ...
  return (tree, file) => {
    ...
    walk(tree, {
      enter(_node) {
        ...
        if (
          ...
          // Content only ever contains Layout
          if (
            isNamedFunction(node, 'Content') &&
            newScope &&
            !inScope(newScope, 'Layout')
          ) {
            fnStack[0].components.push('Layout');
          }
        }
        ...
        if (
          !fnScope ||
          (!isNamedFunction(fnScope.node, '_createContent') &&
        ...
        if (currentScope && node.type === 'JSXElement') {
            ...
            if (!own.call(fnScope.references, fullId)) {
              ...
              if (
                ...
                (parentScope &&
                  ...
                  isNamedFunction(parentScope.node, '_createContent'))
              ) {
          ...
          // But `foo` and `b-ar` are tag names.
            ...
            if (!inScope(currentScope, id)) {
              ...
              // `if` later.
              if (id !== 'Layout' && !own.call(fnScope.references, id)) {
      ...
      leave(node) {
        ...
        if (
          ...
          if (
            ...
            // Accept `components` as a prop if this is the `Content` or
            // `_createContent` function.
            if (
              isNamedFunction(scope.node, 'Content') ||
              isNamedFunction(scope.node, '_createContent')
            ) {
            ...
            // For `['MyComponent', 'Layout']` this generates:
            // ```js
            // const {MyComponent, wrapper: Layout} = _components
            // ```
            // Note that Layout is special as it’s taken from
            // `_components.wrapper`.
            if (actual.length > 0) {
              componentsPattern = {
                type: 'ObjectPattern',
                properties: actual.map((name) => ({
                  ...
                  key: {
                    type: 'Identifier',
                    name: name === 'Layout' ? 'wrapper' : name,
                  },
                  value: { type: 'Identifier', name },
                  method: false,
                  shorthand: name !== 'Layout',
                  computed: false,
                })),
              };
            }
            ...
            if (isNamedFunction(scope.node, '_createContent')) {
              for (const [
                id,
                componentName,
          ...
          while (++index < references.length) {
            ...
            statements.push({
              ...
              consequent: {
                ...
                expression: {
                  type: 'CallExpression',
                  callee: { type: 'Identifier', name: '_missingReference' },
                  arguments: parameters,
                  optional: false,
                },
              },
    ...
    if (createErrorHelper) {
      ...
      tree.body.push({
        ...
        id: { type: 'Identifier', name: '_missingReference' },
        generator: false,
        async: false,
        params: parameters,
        body: {
  ...
  const specifiers = [
    {
      ...
      imported: { type: 'Identifier', name: 'useComponents' },
